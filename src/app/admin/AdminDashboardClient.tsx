...
export default function AdminDashboardClient() {
  const { user } = useUser();
  const { data: doctors = [], isLoading: doctorsLoadind } = useGetDoctors();
  ...
  const stats = {
    ...
    completedAppointment: appointments.filter(
      (app) => app.status === "COMPLETED"
    ).length,
  };

  if (doctorsLoadind || appointmentsLoading) return <LoadingUI />;
